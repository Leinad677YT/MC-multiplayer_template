...
## POSITINAL STUFF
function l.user:inventory/set_pos with storage leinad_temp:player inventory.pos
...
$execute at @s as @e[type=item,tag=temp_check2,distance=..2,limit=1] in l.core:void positioned 0 $(session) 0 run item replace entity @s container.0 from block ~ ~ ~2 container.15

execute at @s run summon item ~ ~ ~ {Item:{id:"stone"},Tags:["temp_check3"]}
$execute at @s as @e[type=item,tag=temp_check3,distance=..2,limit=1] in l.core:void positioned 0 $(session) 0 run item replace entity @s container.0 from block ~ ~ ~2 container.16
