...
## GET QUEUE
    $execute if entity $(lower) run data modify storage leinad_temp:schedule queue set from storage leinad_perm:online session[{lower:"$(lower)"}].schedule
    $execute unless entity $(lower) run data modify storage leinad_temp:schedule queue set from storage z_p$(lower):root data.schedule.time
##
...
## UPDATE QUEUE
    $execute if entity $(lower) run data modify storage leinad_perm:online session[{lower:"$(lower)"}].schedule set from storage leinad_temp:schedule queue
    $execute unless entity $(lower) run data modify storage z_p$(lower):root data.schedule.time set from storage leinad_temp:schedule queue
##
