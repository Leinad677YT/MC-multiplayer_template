...
## SHOULD CLEAR
    ...
    $execute if score #bool l.instance.current matches 1 in $(dimension) store result score #manage_status l.instance.current run function l.instance:manage_clearing with storage leinad_perm:data instance_db[{id:$(id)}]
##

## SHOULD BE MANAGED
    # IF MANAGED => RUN SPECIFIC in $(dimension) ELSE => SET ERROR CODE
    $execute if score #bool l.instance.current matches ..0 in $(dimension) store result score #manage_status l.instance.current run function l.user:instance/typed/$(type)/main with storage leinad_perm:data instance_db[{id:$(id)}]
